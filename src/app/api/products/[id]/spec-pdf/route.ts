...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  function section(title: string) {
    ...
    y -= 4;
    page.drawLine({ start: { x: margin, y }, end: { x: margin + width, y }, thickness: 0.75, color: rgb(0.85, 0.85, 0.85) });
    y -= 16;
  }
  function bodyText(text: string, size = 10.5) {
    for (const raw of text.split("\n")) {
      ...
      for (const line of lines) {
        if (y < margin + 30) { y = 792 - margin; doc.addPage([612, 792]); }
  ...
  if (skus.length) {
    ...
    cols.forEach((c, i) => page.drawText(c, { x: margin + i * colW, y, size: 10, font: bold, color: rgb(0.3, 0.3, 0.3) }));
    y -= 16;
    for (const r of skus) {
      const vals = [r.sku || "—", r.size || "—", r.pack || "—", r.order || "—"];
      vals.forEach((v, i) => page.drawText(String(v), { x: margin + i * colW, y, size: 10, font, color: rgb(0.15, 0.15, 0.15) }));
      y -= 15;
    }
    y -= 8;
  }
